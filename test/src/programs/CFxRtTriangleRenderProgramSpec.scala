...
// MMIO framebuffer from luma-fix-v-sw/samples/baremetal (30x20). Log -> PNG via
...
class CFxRtTriangleRenderProgramSpec
  ...
  private val RenderW = 30
  private val RenderH = 20
  private val DefaultHex = "/samples/c_triangle_render_30x20.hex"
  ...
  describe(s"Baremetal sample [$sampleLabel]") {
    it(
      s"streams 30x20 image through render log [$sampleLabel]",
      Samples
    ) {
